...
def main():
    ...
    # map_data = load_map(map_file) # CTRL + slash
    ...
    create_nn()
    # ウィンドウを作成 --- (*2)
    #★★★★下を修正★★★★★ load_image,set_event 
    create_window(map_data, []) # input
...
def create_window(map_data, events = []):
    ...
    #★★★★下を修正★★★★★
    # g.win.after(movetime, change_label_text) # CTRL+slash
...
def change_label_text():
    ...
    # ret = run_nn()        # CTRL + slash
    # ran = round(ret/0.25) # CTRL + slash
...
def calc_distance_goal():
    distance = (g.goalx-g.px) + (g.goaly-g.py)
...
def load_map(filename):
    ...
    for line in rows:
        ...
        y+=1
        print("")
    if g.goalx == 0 and g.goaly==0:
        print("error ゴール地点が読み込めませんでした。")
        msgbox.showinfo(message="error ゴール地点が読み込めませんでした。")

        sys.exit()
        
    print("g.goalx=",g.goalx, " g.goaly=",g.goaly)
...
def draw_map(cv, data):
    ...
    for y in range(rows):
        ...
        for x in range(cols):
            ...
            #★★★★下を修正★★★★★
            # draw_pic(p, cv, x1, y1) # CTRL + slash
